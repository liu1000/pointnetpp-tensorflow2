...
class PIENET_PT_MODEL(tf.keras.Model):

    def __init__(self, batch_size, num_classes, bn=False, activation=tf.nn.relu):
        super(PIENET_PT_MODEL, self).__init__()
    ...
    def init_network(self):
        ...
        self.dense_clf = layers.Dense(self.num_classes, activation=tf.nn.softmax, name=C.POINT_CLF)
        self.dense_reg = layers.Dense(3, name=C.OFFSET_REG)  # 3D
    ...
    def forward_pass(self, input, training):
        ...
        point_clf_pred = self.dense_clf(net)
        offset_reg_pred = self.dense_reg(net)

        return point_clf_pred, offset_reg_pred
